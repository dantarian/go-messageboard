...
var (
	boardDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Status`: `enum.board_status('open','closed')`, `Description`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)
